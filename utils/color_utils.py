...
#
# Analysis
#   get_category_centroid(category) -> ((r,g,b), '#rrggbb')
#     Centroid averaged in CIELAB (L*, a*, b*); perceptually uniform.
#   summary()
...
class ColorLibrary:
    ...
    def plot_category_hue_lightness(
        ...
        if show_centroid and draw_lines_to_centroid:
            _, c_hex = self.get_category_centroid(category)
            L_c, a_c, b_c = self._hex_to_lab(c_hex)
    ...
    def get_category_centroid(self, category: str) -> Tuple[Tuple[float, float, float], str]:
        """Perceptual centroid of a category, averaged in CIELAB space."""
        hex_list = self.color_library.get(category, [])
        if not hex_list:
        ...
        return self._lab_to_hex(L_sum / n, a_sum / n, b_sum / n)
