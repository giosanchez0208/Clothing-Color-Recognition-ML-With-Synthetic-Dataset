...
license whose defining feature is that *network use counts as distribution*:
...
a camera frame before the color model runs; it is not part of the model, the
...
- *A hosted demo (e.g. HuggingFace Spaces).* If a deployed, network-accessible
  ...
  2. Build the demo without pose detection. Accept an uploaded image and let
     the user select the garment region. The color model itself needs no YOLO,
...
| `matplotlib`, `seaborn` | matplotlib license (BSD-style) / BSD-3-Clause | figures and analysis |
...
Both backbones initialize from ImageNet-pretrained torchvision weights:

- **ResNet-50** (`ResNet50_Weights.IMAGENET1K_V2`), the teacher
- **MobileNetV3-Small** (`MobileNet_V3_Small_Weights.IMAGENET1K_V1`), the student
...
No explicit license is stated on the source pages. The content is colorimetric
...
- <https://huggingface.co/datasets/u5753411/MIT-Indoor-Scenes> (declared MIT license)
...
code from the color library and the background images above. They are
